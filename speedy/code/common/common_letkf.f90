...
SUBROUTINE letkf_core(nobs,nobsl,hdxb,rdiag,rloc,dep,parm_infl,trans)
  ...
  REAL(r_size)              :: rho
!   REAL(r_size)              :: sigma_o, parm(4), gain
!   REAL(r_size),PARAMETER    :: sigma_b = 0.04d0 !error stdev of parm_infl
  ...
  if(verb) call monitor_real_1D(rdiag(1:nobsl),'rdiag')
  if(verb) call monitor_real_1D(rloc(1:nobsl),'rloc')
  if(verb) call monitor_real_2D(hdxb(1:nobsl,:),'hdxb')
  if(verb) call monitor_intg_0D(nobsl,'nobsl')
  ! if(verb) print*,'rdiag',rdiag(1:nobsl)

  if(verb) call print_msg('hdxb Rinv')  
!-----------------------------------------------------------------------
!  hdxb Rinv
!-----------------------------------------------------------------------
  DO j=1,nbv
  ...
  if(verb) call print_msg('hdxb^T Rinv hdxb')
!-----------------------------------------------------------------------
!  hdxb^T Rinv hdxb
!-----------------------------------------------------------------------
...
!  END DO
  ...
  if(verb) call print_msg('covariance inflation')
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  ...
  if(verb) call print_msg('eigenvalues and eigenvectors')
!-----------------------------------------------------------------------
!  eigenvalues and eigenvectors of [ hdxb^T Rinv hdxb + (m-1) I ]
!-----------------------------------------------------------------------
  CALL mtx_eigen(1,nbv,work1,eival,eivec,i)

  if(verb) call print_msg('Pa = [ hdxb^T Rinv hdxb + (m-1) I ]inv')
!-----------------------------------------------------------------------
...
!  END DO
  ...
  if(verb) call print_msg('Pa hdxb_rinv^T')
!-----------------------------------------------------------------------
!  Pa hdxb_rinv^T
!-----------------------------------------------------------------------
...
!  END DO
  ...
  if(verb) call print_msg('Pa hdxb_rinv^T dep')
!-----------------------------------------------------------------------
!  Pa hdxb_rinv^T dep
!-----------------------------------------------------------------------
  DO i=1,nbv
  ...
  if(verb) call print_msg('T = sqrt[(m-1)Pa]')
!-----------------------------------------------------------------------
!  T = sqrt[(m-1)Pa]
!-----------------------------------------------------------------------
  DO j=1,nbv
...
!  END DO
  ...
  if(verb) call print_msg('T + Pa hdxb_rinv^T dep')
!-----------------------------------------------------------------------
...
!  if(verb) call monitor_real_1D(parm,'parm')
!  if(verb) call monitor_real_1D(dep,'dep')
!  if(verb) call monitor_real_1D(rdiag,'rdiag')
!  if(verb) call monitor_real_1D(rloc,'rloc')
  
  
!  if(verb) call print_msg('Inflation estimation')
!!-----------------------------------------------------------------------
!!  Inflation estimation
!!-----------------------------------------------------------------------
!  parm = 0.0d0

!  if(verb) call print_msg('parm(1)')
!  DO i=1,nobsl
!    parm(1) = parm(1) + dep(i)*dep(i)/rdiag(i) * rloc(i)
!  END DO
!  if(verb) call print_msg('parm(2)')
!  DO j=1,nbv
!    DO i=1,nobsl
...
!  if(verb) call print_msg('parm(3)')
!  parm(3) = SUM(rloc(1:nobsl))
!  if(verb) call print_msg('parm(4)')
...
!  if(verb) call monitor_real_1D(parm,'parm')
!  if(verb) call monitor_real_0D(parm_infl,'parm_infl')
