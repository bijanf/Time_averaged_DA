...
das(){
    ...
    long_opts="help,task:,sim_id:,sim_list:,model:,verity:,f90:,debug,clone:,build:,calc:,stat:,plot:,docu:,cpus:,process_time:,mem_per_node:,partition:,verbose:"
    ...
    while true; do
        case "$1" in
            ...
            #--replot      )          replot=yes; shift 1;; ## use when you apply docu to a data-base with outdated plots
            --cpus        )             cpus=$2; shift 2;;
    ...
    case "$task" in
        ...
        "stat"            ) perform_task_in_dataset "stat"  ;; ## statistical calculations
        # "postStat"        ) perform_task_in_dataset "postStat" ;;
        "plot"            ) perform_task_in_dataset "plot"  ;; ## create plots from stats
        "docu"            ) perform_task_in_dataset "docu"  ;; ## create a pdf of the results
        "view"            ) perform_task_in_dataset "view"  ;; ## opens the docu 
...
perform_task_in_dataset(){
    ...
    ${cfg[dataset_kind]}.sh --task=$task  "$dataset_dir" #modified by Bijan :  --replot=${replot:-} is deleted
}
...
#     --output="$log_file" \
#     --error="${log_file}.out" \
#     --mail-user=bijan.fallah@met.fu-berlin.de \
#     --mail-type=end \
#     --ntasks=$cpus \
...
queue_in_met_fub(){
    ssh calc02 ". /etc/profile; . ~/.bashrc; $CODE_DIR/das.sh --task=queue_met_fub --model=$model --sim_id=$sim_id --cpus=$cpus"; }
...
queue_in_soroban(){
    ...
    ### ssh soroban "mkdir -p $ARCH_THERE" [modified by Bijan]
    echo ${ARCH_THERE}
    ssh bijanf@soroban.zedat.fu-berlin.de "mkdir -p $ARCH_THERE" #[modified by Bijan]
    scp "$ARCH_DIR/tmp/tmp_code.tar.gz" "bijanf@soroban.zedat.fu-berlin.de:$ARCH_THERE" > /dev/null
    #ssh soroban "cd $ARCH_THERE; tar -xvf tmp_code.tar.gz > /dev/null" #[modified by Bijan]
    ssh bijanf@soroban.zedat.fu-berlin.de "cd $ARCH_THERE; tar -xvf tmp_code.tar.gz > /dev/null" #[modified by Bijan]

    echo " - Submiting process to queue"
    #ssh soroban "export PATH=/home/bijanf/bin:\$PATH; $ARCH_THERE/das_copy${count}/code/das.sh --task=queue_soroban --clone=no --model=$model --sim_id=$sim_id" #[modified by Bijan]
    ssh bijanf@soroban.zedat.fu-berlin.de "export PATH=/home/bijanf/bin:\$PATH; $ARCH_THERE/das_copy${count}/code/das.sh --task=queue_soroban --clone=no --model=$model --sim_id=$sim_id" #[modified by Bijan]
    funct_closing 6
}
