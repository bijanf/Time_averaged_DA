...
calculate_ensemble_stats(){
    ...
    (   cd $dataset_dir/raw_data/$Ephase
        ...
        mpirun -n $cpus Emean_spread.exe < /dev/null
...
error_increase_regarding_linear_obs(){
   ...
   # assi_run_norm_addObs_dir="../${cfg[rel_assi_run_normAdd_obs_dir]}/stats"
   assi_run_norm_addObs_dir="../../assi_run__m24__hLoc500km__InfFac1__stationSet8__obs_op_norm_add/stats"
...
error_reduction_regarding(){
    ...
    refRunId="$1"; refRunEphase="$2"; refRunDir="$3";
    stdRunEphase="$4"
    stdRunDir="$dataset_dir"

    read refRunType < "$refRunDir/config/run_type.cfg"
    read stdRunType < "$stdRunDir/config/run_type.cfg"

    cd "$dataset_dir/stats"
    stat_id="ReducRef${refRunId}"

    for Tkind in ${Tkinds[@]}; do
        stat_name="error_grid_sigma_${Tkind}_timstd1"
        Decrease               "${stdRunType}_${stdRunEphase}_${stat_name}" \
	    "${refRunDir}/stats/${refRunType}_${refRunEphase}_${stat_name}" \
	    "${stat_id}"
        reduceDimensionality . . "${stdRunType}_${stdRunEphase}_${stat_name}_${stat_id}"
    done

    funct_closing 2
}

error_comparison_regarding(){
    funct_opening 2

    refRunId="$1";
    refRunEphase="$2"; refRunDir="$3";
    stdRunEphase="$4"; stdRunDir="$dataset_dir"
    ...
    for operator in Decrease Increase; do
	for Tkind in ${Tkinds[@]}; do
	    # for stdRunEphase in prior postr; do
		stat_id="${operator}Ref${refRunId}_${refRunEphase}"
		stat_name="error_grid_sigma_${Tkind}_timstd1"
		$operator              "${stdRunType}_${stdRunEphase}_${stat_name}" \
		    "${refRunDir}/stats/${refRunType}_${refRunEphase}_${stat_name}" \
		    "${stat_id}"
		reduceDimensionality . . "${stdRunType}_${stdRunEphase}_${stat_name}_${stat_id}"
	    # done
