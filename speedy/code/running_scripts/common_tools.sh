...
calling_function(){
    info=$(caller 0)        # Calling function info ## modified by Bijan
    OLD_IFS="$IFS"; IFS=" " # setting temporally info separator
    info_array=( $info )
    IFS="$OLD_IFS"          # restoring original file separator
...
CDO_LAND(){
	...
	if [[ $oper1 == "import_binary" ]];then
	    ...
	    #my_cdo ${oper1} -sellonlatbox,0,360,-90,90 -ifthen -invertlat "/daten/cady/das_01/speedy/code/speedy_ver32/data/bc/t30/clim/land_sea_3.nc" "${inDir}/${filename}.nc" "$outDir/${filename}_${oper1}.nc" #modified by BIJAN
	     my_cdo ${oper1} -sellonlatbox,0,360,0,90 "${inDir}/${filename}.nc" "$outDir/${filename}_${oper1}.nc" #modified by BIJAN
	fi
    fi
...
setup_modules_system(){
    ...
    # module load modules
    . /net/opt/system/modules/default/init/bash
}
