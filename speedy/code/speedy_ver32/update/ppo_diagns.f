...
C--   3. Stop integration if model variables are out of range
     ...
     &      DIAG(K,2).GT.500.OR.
     &      DIAG(K,3).LT.180.OR.DIAG(K,3).GT.320.) THEN
...
C           Write model fields at t-1 on output file 
            !IF (IHOUT.NE.1) THEN !Only when no hourly output            !TM
            !  CALL TMOUT (0)
            !  CALL TMINC
            !  NSTOUT=NSTPPR
            !  CALL TMOUT (1)
            ...
            PRINT *,'*** Model variables out of accepted range ***'
c            STOP '*** Model variables out of accepted range ***'
