...
C--:   IHOUT  = 6-hourly output flag                (0=no, 1=yes)        !TM
C--:   IPOUT  = 6-hourly pressure level output flag (0=no, 1=yes)   
C--:   NMONRS = Period (no. of months) for restart file update
C--:   ISEASC = Seasonal cycle flag (0=no, 1=yes)
C--:   ISTART = Input restart file mode
C--:   JSTART = Output_flag
...
C--:   IMONT0 = Month of initial date (1 to 12)
     ...
     &                NMONRS,  IHOUT,  IDOUT,  IPOUT, IYEAR0, IMONT0,
     &                ISEASC, ISTART, JSTART

!    &                NSTDIA, NSTPPR, NSTOUT, IDOUT, NMONRS,            !TM
!    &                ISEASC, ISTART, IYEAR0, IMONT0                    !TM
...
C--:   IALST  = anomaly flag for land-surface temp. (0=no, 1=slab-model)
C--:   IASST  = anomaly flag for sea-surface temp.  
C--:            (0=no, 1=prescribed, 2=slab-model, 3=prescr.+slab-model)
C--:   IAICE  = anomaly flag for sea-ice temp. (0=no, 1=slab-model)
C--    ISSTAN = prescribed SST anomaly flag (0=no, 1=yes)
C--:   ISST0  = record in SST anomaly file corr. to the initial month     

      COMMON /ISTFOR/ NSTRAD, NSTRDF, INDRDF,
     &                IALST, IASST, IAICE, ISSTAN, ISST0
...
C--    ALPH   = Coefficient for semi-implicit computations
      ...
      COMMON /RSTEPS/ DELT, DELT2, ROB, ALPH
