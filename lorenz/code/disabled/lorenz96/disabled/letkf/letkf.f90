...
!-----------------------------------------------------------------------
  ...
  PRINT '(A,F12.5)','INFL = ',inflave
	...
	OPEN(10,FILE='rmse_4byte.grd',FORM='unformatted')
	DO i=1,nt
		tmp_4 = REAL(rmse_t(i),r_sngl);  WRITE(10) tmp_4
	END DO
	CLOSE(10)
	CALL set_time_series_ctl('rmse_4byte',it-1,'RMSE')

	OPEN(10,FILE='spread_4byte.grd',FORM='unformatted')
	DO i=1,nt
		tmp_4 = REAL(sprd_t(i),r_sngl); WRITE(10) tmp_4
	END DO
	CLOSE(10)
	CALL set_time_series_ctl('spread_4byte',it-1,'spread')
	
  STOP
END PROGRAM letkf

SUBROUTINE set_time_series_ctl(filename,snapshots,varname)
	...
	WRITE(11,'(A)'     )'VARS 1'
	WRITE(11,'(2A)'    ) varname, ' 1 99 RMSE'
	WRITE(11,'(A)'     )'ENDVARS'
	CLOSE(11)
	
	RETURN
END SUBROUTINE set_time_series_ctl
