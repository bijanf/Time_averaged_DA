...
function time_series(cfg,state)
    ...
    for j=1:2
        subplot(2,1,j);
        hold on;
        plot(state.dim.data, squeeze(state.var(j).data(1,1,1,:)),'b');
        plot(state.dim.data, squeeze(state.var(j).data(2,1,1,:)),'r');
        plot(state.dim.data, squeeze(state.var(j).data(3,1,1,:)),'g');
