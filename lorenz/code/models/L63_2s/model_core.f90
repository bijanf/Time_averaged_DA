...
MODULE model_core
  ...
  ! Spatial distribution of model state vector (Relevant for spatial localization)
  INTEGER      ,PARAMETER:: var_pos (nx)       =(/0,0,0,0,0,0/)
  ! INTEGER                :: i_pos ,j_pos
  ! !  INTEGER ,PARAMETER :: var_comp(nx)=(/((i_pos+nc*(j_pos), i_pos=1,nc),j_pos=0,2,2)/)

  ! Component distribution of model state vector (Relevant for component localization)
  INTEGER      ,PARAMETER:: var_comp(nx)       =(/1,1,1,2,2,2/)
  ...
  CHARACTER(5),PROTECTED:: isotropic_coupling ! 'yes','no'

  ! Original Pena & Kalnay coupling configurations
  !  %c = 0.00; cz = 0.0; S = 1.0; k1 = 10; % Uncoupled
  ...
  !  INTEGER      ,PARAMETER :: station_pos(ny)=(/(i_pos, i_pos=2,3)/)
...
CONTAINS
  ...
  SUBROUTINE model_get_par_values
    ...
    CALL get_string_par_env ('isotropic_coupling',isotropic_coupling)

    SELECT CASE (TRIM(isotropic_coupling))
    CASE ('yes')
       cz = c
    CASE DEFAULT
