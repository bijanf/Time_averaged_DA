...
function obs_time_series(cfg,state)
    ...
    for l=1:2
        for m=1:length(state.value.var(l).data(:,1,1,1))
            ...
            resp{l}(m,:) = response_function(state_var{l},threshold_down{l},threshold_up{l});
        end
    end
...
% %            state_min{1,j}(j,1),state_max{1,j}(j,1));
% 
%         hold on;
%         plot(time,       resp{1,j+1}(:),'r');
%         plot(time,       resp{2,j+1}(:),'b');
