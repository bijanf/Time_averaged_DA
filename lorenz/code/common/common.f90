MODULE common
  ...
  ! Constants
  !-----------------------------------------------------------------------
  REAL(r_dble),PARAMETER :: pi=3.1415926535d0
  REAL(r_dble),PARAMETER :: gg=9.81d0
  REAL(r_dble),PARAMETER :: rd=287.0d0
  REAL(r_dble),PARAMETER :: cp=1005.7d0
  REAL(r_dble),PARAMETER :: re=6371.3d3
  REAL(r_dble),PARAMETER :: r_omega=7.292d-5
  REAL(r_dble),PARAMETER :: t0c=273.15d0
  REAL(r_dble),PARAMETER :: undef=-9.99d33
...
CONTAINS
  ...
  SUBROUTINE com_mean(ndim,var,amean)
    ...
    REAL(r_dble),INTENT(IN) :: var(ndim)
    REAL(r_dble),INTENT(OUT):: amean
  ...
  SUBROUTINE com_stdev(ndim,var,aout)
    ...
    INTEGER,INTENT(IN) :: ndim
    REAL(r_dble),INTENT(IN) :: var(ndim)
    REAL(r_dble),INTENT(OUT) :: aout

    REAL(r_dble) :: amean
    REAL(r_dble) :: dev(ndim)

    CALL com_mean(ndim,var,amean)

    dev(:) = var(:) - amean

    aout = SQRT( SUM(dev*dev) / REAL(ndim-1,r_dble) )
  ...
  SUBROUTINE com_covar(ndim,var1,var2,cov)
    ...
    INTEGER,INTENT(IN) :: ndim
    REAL(r_dble),INTENT(IN) :: var1(ndim)
    REAL(r_dble),INTENT(IN) :: var2(ndim)
    REAL(r_dble),INTENT(OUT) :: cov

    REAL(r_dble) :: amean1,amean2
    REAL(r_dble) :: dev1(ndim),dev2(ndim)
    ...
    cov = SUM( dev1*dev2 ) / REAL(ndim-1,r_dble)
  ...
  SUBROUTINE com_correl(ndim,var1,var2,cor)
    ...
    REAL(r_dble),INTENT(IN) :: var1(ndim)
    REAL(r_dble),INTENT(IN) :: var2(ndim)
    REAL(r_dble),INTENT(OUT) :: cor

    REAL(r_dble) :: cov,stdev1,stdev2
  ...
  SUBROUTINE com_anomcorrel(ndim,var1,var2,varmean,cor)
    ...
    REAL(r_dble),INTENT(IN) :: var1(ndim)
    REAL(r_dble),INTENT(IN) :: var2(ndim)
    REAL(r_dble),INTENT(IN) :: varmean(ndim)
    REAL(r_dble),INTENT(OUT) :: cor
  ...
  SUBROUTINE com_rms(ndim,var,rmsv)
    ...
    INTEGER,INTENT(IN) :: ndim
    REAL(r_dble),INTENT(IN) :: var(ndim)
    REAL(r_dble),INTENT(OUT) :: rmsv

    rmsv = SQRT( SUM(var*var) / REAL(ndim,r_dble) )
  ...
  SUBROUTINE com_filter_lanczos(ndim,fc,var)
    ...
    INTEGER,INTENT(IN) :: ndim
    REAL(r_dble),INTENT(IN) :: fc    ! critical frequency in [0,pi]
    REAL(r_dble),INTENT(INOUT) :: var(ndim)

    INTEGER,PARAMETER :: lresol=10

    REAL(r_dble) :: weight(-lresol:lresol)
    REAL(r_dble) :: varwk(1-lresol:ndim+lresol)
    REAL(r_dble) :: rl,rlresol
    INTEGER :: i,l
    !
    ! Weight
    !
    rlresol = REAL(lresol,r_dble)
    DO l=-lresol,-1
       rl = REAL(l,r_dble)
  ...
  SUBROUTINE com_distll(ndim,alon,alat,blon,blat,dist)
    IMPLICIT NONE
    INTEGER,INTENT(IN) :: ndim
    REAL(r_dble),INTENT(IN) :: alon(ndim)
    REAL(r_dble),INTENT(IN) :: alat(ndim)
    REAL(r_dble),INTENT(IN) :: blon(ndim)
    REAL(r_dble),INTENT(IN) :: blat(ndim)
    REAL(r_dble),INTENT(OUT) :: dist(ndim)
    REAL(r_dble),PARAMETER :: r180=1.0d0/180.0d0
    REAL(r_dble) :: lon1,lon2,lat1,lat2
    REAL(r_dble) :: cosd(ndim)
    INTEGER :: i
  ...
  SUBROUTINE com_distll_1(alon,alat,blon,blat,dist)
    IMPLICIT NONE
    REAL(r_dble),INTENT(IN) :: alon
    REAL(r_dble),INTENT(IN) :: alat
    REAL(r_dble),INTENT(IN) :: blon
    REAL(r_dble),INTENT(IN) :: blat
    REAL(r_dble),INTENT(OUT) :: dist
    REAL(r_dble),PARAMETER :: r180=1.0d0/180.0d0
    REAL(r_dble) :: lon1,lon2,lat1,lat2
    REAL(r_dble) :: cosd
  ...
  SUBROUTINE com_interp_spline(ndim,x,y,n,x5,y5)
    ...
    REAL(r_dble),INTENT(IN) :: x(ndim) ! coordinate
    REAL(r_dble),INTENT(IN) :: y(ndim) ! variable
    INTEGER,INTENT(IN) :: n            ! number of targets
    REAL(r_dble),INTENT(IN) :: x5(n)   ! target coordinates
    REAL(r_dble),INTENT(OUT) :: y5(n)  ! target values
    INTEGER :: i,j,m
    REAL(r_dble) :: dydx(5),ddydx(4),t(2),dx21,dx
    REAL(r_dble) :: wk
  ...
  SUBROUTINE com_pos2ij(msw,nx,ny,flon,flat,num_obs,olon,olat,oi,oj)
    ...
    REAL(r_dble),INTENT(IN) :: flon(nx,ny),flat(nx,ny) !(lon,lat) at (i,j)
    INTEGER,INTENT(IN) :: num_obs !repetition number of conversion
    REAL(r_dble),INTENT(IN) :: olon(num_obs),olat(num_obs) !target (lon,lat)
    REAL(r_dble),INTENT(OUT) :: oi(num_obs),oj(num_obs) !target (i,j)
    ...
    INTEGER :: nxp,nyp
    REAL(r_dble),PARAMETER :: miss = -32768
    REAL(r_dble),PARAMETER :: max_dist = 2.0e+6
    REAL(r_dble) :: rlat_max, rlat_min, rlon_max, rlon_min
    REAL(r_dble) :: dist(num_grid_ave)
    REAL(r_dble) :: dist_min_x(num_obs, num_grid_ave)
    REAL(r_dble) :: dist_min_y(num_obs, num_grid_ave)
    REAL(r_dble) :: wk_dist, sum_dist
    REAL(r_dble) :: ratio(num_grid_ave)
    IF(detailout) THEN
       WRITE(6,'(A)') '====================================================='
  ...
  SUBROUTINE com_utc2tai(iy,im,id,ih,imin,sec,tai93)
    ...
    REAL(r_dble),INTENT(IN) :: sec
    REAL(r_dble),INTENT(OUT) :: tai93
    REAL(r_dble),PARAMETER :: mins = 60.0d0
    REAL(r_dble),PARAMETER :: hour = 60.0d0*mins
    REAL(r_dble),PARAMETER :: day = 24.0d0*hour
    REAL(r_dble),PARAMETER :: year = 365.0d0*day
    INTEGER,PARAMETER :: mdays(12) = (/31,28,31,30,31,30,31,31,30,31,30,31/)
    INTEGER :: days,i

    tai93 = REAL(iy-1993,r_dble)*year + FLOOR(REAL(iy-1993)/4.0,r_dble)*day
    days = id -1
    DO i=1,12
       IF(im > i) days = days + mdays(i)
    END DO
    IF(MOD(iy,4) == 0 .AND. im > 2) days = days + 1 !leap year
    tai93 = tai93 + REAL(days,r_dble)*day + REAL(ih,r_dble)*hour &
         & + REAL(imin,r_dble)*mins + sec
  ...
  SUBROUTINE com_tai2utc(tai93,iy,im,id,ih,imin,sec)
    ...
    INTEGER,PARAMETER :: leapsec(n) = (/  15638399,  47174400,  94608001,&
         ...
         &    504921605, 615254406/)
    REAL(r_dble),INTENT(IN) :: tai93
    INTEGER,INTENT(OUT) :: iy,im,id,ih,imin
    REAL(r_dble),INTENT(OUT) :: sec
    REAL(r_dble),PARAMETER :: mins = 60.0d0
    REAL(r_dble),PARAMETER :: hour = 60.0d0*mins
    REAL(r_dble),PARAMETER :: day = 24.0d0*hour
    REAL(r_dble),PARAMETER :: year = 365.0d0*day
    ...
    sec = 0.0d0
    DO i=1,n
       IF(FLOOR(tai93) == leapsec(i)+1) sec = 60.0d0 + tai93-FLOOR(tai93,r_dble)
       IF(FLOOR(tai93) > leapsec(i)) tai = tai -1.0d0
    END DO
    iy = 1993 + FLOOR(tai /year)
    wk = tai - REAL(iy-1993,r_dble)*year - FLOOR(REAL(iy-1993)/4.0,r_dble)*day
    IF(wk < 0.0d0) THEN
       iy = iy -1
       wk = tai - REAL(iy-1993,r_dble)*year - FLOOR(REAL(iy-1993)/4.0,r_dble)*day
    END IF
    days = FLOOR(wk/day)
    wk = wk - REAL(days,r_dble)*day
    im = 1
    DO i=1,12
       leap = 0
    ...
    ih = FLOOR(wk/hour)
    wk = wk - REAL(ih,r_dble)*hour
    imin = FLOOR(wk/mins)
    IF(sec < 60.0d0) sec = wk - REAL(imin,r_dble)*mins
