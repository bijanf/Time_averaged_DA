...
test_list(){
    ...
    verity=${verity:-0}
    verbose=${verbose:-1}
    source "$CODE_DIR/models/$model/model_config.sh"
...
test_individual_runs(){
    ...
    run.sh --run_mode=free --detailed_stats=yes --verbose=1 \
        "$run_free_dir" || error
    ...
    update_mode='Augm0'; run_assi_dir_B="$(run_assi_dir)"
    run.sh --run_mode=assi --detailed_stats=yes \
        --ref_dir="$run_free_dir" \
...
test_run_free_set1D(){
    ...
    run_free_set1D_1_dir=$test_dir/run_free_set1D_1
    run_set.sh --run_mode=free --detailed_stats=yes --run_verbose=0 \
        --par1=cycle_length --span1="ispan(5,8,1)*$dt" \
        "$run_free_set1D_1_dir" || error
...
test_run_assi_set1D(){
    ...
    update_mode='Augm0'; run_assi_set1D_2_dir=$test_dir/run_assi_set1D_2
    run_set.sh --run_mode=assi --ref_dir="$run_free_dir" \
        --par1=SNR --span1="fspan(2,10,2)" \
